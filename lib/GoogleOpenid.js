const express = require('express');
const openid = require('openid');
const GOOGLE_ENDPOINT = 'https://www.google.com/accounts/o8/id';

var GoogleOpenid = function(app, callback, onAuthentication) {
    this.callback = callback;
    var that = this;
    
    app.get('/googleopenid/verify', function(req, res) {
        that.verify(req, res, onAuthentication);
    });
    
    app.get('/googleopenid/authenticate', function(req, res) {
        that.authenticate(res);
    });
}

GoogleOpenid.prototype.verify = function(req, res, onAuthentication) {
    var openid_result = openid.verifyAssertion(req.url); // or req.url
    onAuthentication(req, res, openid_result);
}

GoogleOpenid.prototype.authenticate = function(res) {
    openid.authenticate(
        GOOGLE_ENDPOINT,
        this.callback, // our callback URL
        null, // realm (optional)
        false, // attempt immediate authentication first?
        function(authUrl) {
            res.writeHead(302, { Location: authUrl });
            res.end();
        }
    );
}

module.exports.create = function(app, callback, onAuthentication) {
    return new GoogleOpenid(app, callback, onAuthentication);
}
